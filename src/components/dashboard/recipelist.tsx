              ...
              <div className="min-w-0">
                <div className="flex items-start gap-x-3">
                  <Link 
                    ...
                    className="text-base sm:text-lg font-semibold leading-6 text-gray-700 hover:text-gray-500">{recipe.status == "Complete" ? recipe.name : recipe.status == "Failed" ? "Error getting recipe. Please try again later" : "Loading, please wait..."}</Link>
                    <p
                    className={
                ...
                <div className="mt-1 flex items-center gap-x-2 text-xs sm:text-sm leading-5 text-gray-500">
                  <p className={recipe?.time ? `whitespace-nowrap` : `hidden`}>
                    <span>{recipe ? `${recipe?.time} Minutes` : ''}</span>
                  </p>
                  <svg viewBox="0 0 2 2" className={recipe?.time ? `h-0.5 w-0.5 fill-current` : `hidden`}>
                    <circle cx={1} cy={1} r={1} />
                  </svg>
                  <p className="whitespace-nowrap">
                    <span>{recipe ? recipe.ingredients.length : '0'} Ingredients</span>
                  </p>
                  <svg viewBox="0 0 2 2" className="h-0.5 w-0.5 fill-current">
                    <circle cx={1} cy={1} r={1} />
                  </svg>
                  <p className="whitespace-nowrap">
                    <span>{recipe ? recipe.instructions.length : '0'} Steps</span>
                  </p>
